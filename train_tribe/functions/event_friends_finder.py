...
def get_event_trip_friends_logic(params):
    ...
    logging.error("Starting event trip friends logic...")
    ...
    logging.error(f"Found {len(friends)} friends for user {user_id}.")
    ...
    for route in event_routes:
        for leg_key, leg in route.items():
            if leg_key.startswith("leg"):
                logging.error(f"Checking leg {leg_key} for trip_id {leg.get('trip_id')}...")
                friends_on_trip = check_friends_on_trip(leg.get("trip_id"), friends, date)
                if friends_on_trip:
                    leg["friends"] = friends_on_trip
                    logging.error(f"Found {len(friends_on_trip)} friends on trip {leg.get('trip_id')}.")
    ...
    logging.error(f"Event trip friends logic completed. New file uploaded to {new_event_options_path}")
...
def check_friends_on_trip(trip_id, friends, date):
    ...
    logging.error(f"Found {len(users_on_trip)} users on trip {trip_id}.")

    friends_on_trip = []
    
    for user_on_trip in users_on_trip:
        logging.error(f"Checking user {user_on_trip.id} on trip {trip_id}...")
